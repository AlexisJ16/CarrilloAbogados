...
public interface CaseActivityRepository extends JpaRepository<CaseActivity, Integer> {
    ...
    @Query("SELECT ca FROM CaseActivity ca WHERE ca.deadline IS NOT NULL " +
            "AND ca.deadline < :now AND ca.isCompleted = false " +
            "ORDER BY ca.deadline ASC")
    List<CaseActivity> findOverdueActivities(@Param("now") LocalDateTime now);

    /**
     * Find upcoming deadlines within a date range
     */
    @Query("SELECT ca FROM CaseActivity ca WHERE ca.deadline IS NOT NULL " +
            "AND ca.deadline >= :now AND ca.deadline <= :endDate AND ca.isCompleted = false " +
            "ORDER BY ca.deadline ASC")
    List<CaseActivity> findUpcomingDeadlines(@Param("now") LocalDateTime now, @Param("endDate") LocalDateTime endDate);

    /**
     * Find milestones by case ID
     */
    @Query("SELECT ca FROM CaseActivity ca WHERE ca.legalCase.caseId = :caseId AND ca.isMilestone = true ORDER BY ca.activityDate DESC")
    List<CaseActivity> findMilestonesByCaseId(@Param("caseId") Integer caseId);
    ...
    Double getTotalBillableHoursByCaseId(@Param("caseId") Integer caseId);
    ...
    Double getTotalCostByCaseId(@Param("caseId") Integer caseId);
    ...
    Double getTotalHoursByCaseId(@Param("caseId") Integer caseId);
