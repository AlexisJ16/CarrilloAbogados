...
┌──────────────────┐      ┌──────────────────┐      ┌──────────────────┐
│     SUB-B        │      │     SUB-C        │      │     SUB-D        │
│  Hot Lead        │      │    Instant       │      │   Nurturing      │
│  Notification    │      │    Response      │      │    Sequence      │
│                  │      │                  │      │    Engine        │
│ • Email Dr.      │      │ • Email IA       │      │ • 8-12 emails    │
│ • SMS (opcional) │      │ • Personalizado  │      │ • Scheduling     │
│ • WhatsApp       │      │ • Inmediato      │      │ • A/B testing    │
│ • Slack          │      └──────────────────┘      └──────────────────┘
└──────────────────┘
```
...
| ID | Componente | Tipo | Trigger | Función Principal | Criticidad |
|----|------------|------|---------|-------------------|------------|
| **A** | Lead Lifecycle Manager | Orquestador (HUB) | Webhook `/lead-events` | Recibir y enrutar eventos | CRÍTICO |
| **SUB-A** | Lead Intake & Enrichment | Spoke | Execute Workflow | Capturar, validar, clasificar, responder | CRÍTICO |
| **SUB-B** | Hot Lead Notification | Spoke | Execute Workflow | Notificar equipo de leads HOT | ALTO |
| **SUB-C** | Instant Response | Spoke | Execute Workflow | Email personalizado IA | ALTO |
| **SUB-D** | Nurturing Sequence | Spoke | Schedule (6h) | Secuencia de emails automatizada | MEDIO |
| **SUB-E** | Engagement Tracker | Spoke | Webhook Mailersend | Tracking opens/clicks | MEDIO |
| **SUB-F** | Meeting Scheduler | Spoke | Webhook Calendly | Sincronizar reuniones agendadas | ALTO |
...
#### ¿Por qué SUB-B es un workflow separado?

| Factor | Decisión |
|--------|----------|
| **Fragilidad** | Notificación puede fallar (SMS, WhatsApp) sin afectar la captura |
| **Opcional** | Solo se ejecuta para leads HOT |
| **Modificabilidad** | Canales de notificación cambian frecuentemente |
| **Testing** | Más fácil testear notificaciones aisladamente |
...
|------------|--------|------------|-------|
| WORKFLOW A (Orquestador) | 🟢 Activo | 100% | URL webhook lista |
| SUB-A: Lead Intake | 🟡 Validado | 100% | Pendiente activación |
| SUB-B: Hot Notification | 📋 Especificado | 70% | Spec completa, impl pendiente |
| SUB-C: Instant Response | ✅ Integrado en SUB-A | 100% | - |
| SUB-D: Nurturing | ⚪ Planificado | 10% | Diseño inicial |
